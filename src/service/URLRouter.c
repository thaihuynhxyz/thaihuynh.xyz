...
struct http_result *Api_contact_handle_get() {
    log_info("func %s - conn", __func__);
    struct http_result *result = malloc(sizeof(struct http_result));
    result->http_code = MHD_HTTP_OK;
    char *buffer = NULL;
    ...
    char *current_dir = get_current_dir();
    char *path = malloc(strlen(current_dir) + strlen(PAGE_CONTACT) + 1);
    strcpy(path, current_dir);
    strcat(path, PAGE_CONTACT);
...
struct http_result *Api_contact_handle(struct connection_info *info) {
    if (info->connection_type == GET) return Api_contact_handle_get();
...
void *URLRouter_create() {
    ...
    router->routes = TSTree_insert(router->routes, API_CONTACT, sizeof(API_CONTACT), Api_contact_handle);
...
struct http_result *URLRouter_route(URLRouter *self, const char *url, struct connection_info *info) {
    ...
    log_info("func %s: url=%s", __func__, url);
    struct http_result *result = malloc(sizeof(struct http_result));
